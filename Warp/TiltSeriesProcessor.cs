...
namespace Warp
{
    ...
    public class TiltSeriesProcessor
    {
                    ...
                    using (var stream = sshclient.CreateShellStream("xterm", 80, 50, 1024, 1024, 1024))
                    {
                        ...
                        while (stream.CanRead)
                        {
                            ...
                            else if (line.StartsWith("New tilt axis:"))
                            {
                                ...
                                Console.Write($"Aretomo refined tilt axis: {ts.AretomoRefinedTiltAxis}");
                            }
        ...
        public static jobStatus RunAretomo2Png(TiltSeriesViewModel ts, OptionsSshSettings settings)
        {
            if (!File.Exists(ts.MdocFile)) { LogToFile($"Aretomo2png failed: {ts.MdocFile} does not exist.");  return jobStatus.Failed; }

            string baseDir = Path.GetDirectoryName(ts.MdocFile);
            string baseName = Path.GetFileNameWithoutExtension(ts.MdocFile);
            if (baseDir == null || baseName == null) { LogToFile($"Aretomo2png failed: {baseDir} or {baseName} is null."); return jobStatus.Failed; }
            string LinuxPath = settings.LinuxPath.TrimEnd('/') + "/aretomo/" + baseName;

            var files = ProcessingFiles(ts.MdocFile, settings.LinuxPath);

            if (!File.Exists(files.Projection)) { LogToFile($"Aretomo2png failed: {files.Projection} does not exist.");  return jobStatus.Failed; }

            string cmd = String.Format("mrc2tif -j -a 0,0 {0} {1}; ", files.ProjectionLinux, files.OutPngLinux);
            cmd += String.Format("mrc2tif -j -a 0,0 {0} {1}; echo DONE", files.ProjectionXZLinux, files.OutPngXZLinux);
            ...
            if (File.Exists(files.AretomoOutSt)) {
                ...
                //{
                    ...
                    cmd += $"mrc2tif -j {LinuxPath}/{baseName}_aln_st.mrc {LinuxPath}/{baseName}_aln_st; ";
            ...
            if (File.Exists(files.OutTomo)) {
                ...
                cmd = $"mrc2tif -j {files.OutTomoLinux} {LinuxPath}/{baseName}; ";
                cmd += $"ffmpeg -y -i {LinuxPath}/{baseName}.%03d.jpg -pix_fmt yuv420p -s 452x640 {files.AretomoOutTomogramMovLinux}; rm {LinuxPath}/{baseName}.*.jpg; "; //Run ffmpeg
                cmd += "echo DONE";
                LogToFile(cmd);
                try
                {
            ...
            int c = 0;
            while (!File.Exists(files.OutPng))
            {
                Thread.Sleep(1000);
                c++;
                if (c > 5)
